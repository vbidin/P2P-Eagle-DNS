/**
 * $Id: ExchangeMessage.java,v 1.2 2005/11/07 16:56:38 rschmidt Exp $
 *
...
/**
 * This class represents a Gridella exchange message.
 *
 * @author @author <a href="mailto:Roman Schmidt <Roman.Schmidt@epfl.ch>">Roman Schmidt</a>
 * @version 1.0.0
 */
public class ExchangeMessage extends Exchange implements PGridMessage, XMLizable {

	/**
	 * A part of the XML string.
	 */
	public static final String XML_EXCHANGE = "Exchange";

	/**
	 * A part of the XML string.
	 */
	private static final String XML_EXCHANGE_GUID = "GUID";

	/**
	 * A part of the XML string.
	 */
	private static final String XML_EXCHANGE_LEN_CURRENT = "CurrentLength";

	/**
	 * A part of the XML string.
	 */
	private static final String XML_EXCHANGE_RANDOM_NUMBER = "RandomNumber";

	/**
	 * A part of the XML string.
	 */
	private static final String XML_EXCHANGE_RECURSION = "Recursion";

	/**
	 * A part of the XML string.
	 */
	private static final String XML_EXCHANGE_MINSTORAGE = "MinStorage";

	/**
	 * A part of the XML string.
	 */
	private static final String XML_EXCHANGE_REPLICA_ESTIMATE = "ReplicaEstimate";
	 ...
	 */
	public ExchangeMessage(MessageHeader header) {
		super();
		mHeader = header;
	 ...
	 * @param sign         the signature of the data table.
	 */
	public ExchangeMessage(GUID guid, PGridHost host, int recursion, int lCurrent, int minStorage, double replicaEst,
	 ...
	 */
	public int getDesc() {
		return PGridMessage.DESC_EXCHANGE;
	}
	...
	public String getDescString() {
		return PGridMessage.DESC_EXCHANGE_STRING;
	}
	...
	public synchronized void startElement(String uri, String lName, String qName, Attributes attrs) throws SAXException {
		if (qName.equals(XML_EXCHANGE)) {
			// Exchange
			String guidStr = attrs.getValue(XML_EXCHANGE_GUID);
			if (guidStr != null)
				mGUID = new pgrid.GUID(attrs.getValue(XML_EXCHANGE_GUID));
			String recStr = attrs.getValue(XML_EXCHANGE_RECURSION);
			if (recStr == null)
				mRecursion = 0;
			else
				mRecursion = Integer.parseInt(recStr);
			String lCurrStr = attrs.getValue(XML_EXCHANGE_LEN_CURRENT);
			if (lCurrStr == null)
				mLenCurrent = 0;
			else
				mLenCurrent = Integer.parseInt(lCurrStr);
			String minStorage = attrs.getValue(XML_EXCHANGE_MINSTORAGE);
			if (minStorage == null)
				mMinStorage = 0;
			else
				mMinStorage = Integer.parseInt(minStorage);
			String rndNmbrStr = attrs.getValue(XML_EXCHANGE_RANDOM_NUMBER);
			if (rndNmbrStr == null)
				mRandomNumber = Double.MIN_VALUE;
			else
				mRandomNumber = Double.parseDouble(rndNmbrStr);
			String replicaEstStr = attrs.getValue(XML_EXCHANGE_REPLICA_ESTIMATE);
	...
	public synchronized String toXMLString(String prefix, String newLine) {
		...
		strBuff.append(prefix + XML_ELEMENT_OPEN + XML_EXCHANGE); // {prefix}<Exchange
		strBuff.append(XML_SPACE + XML_EXCHANGE_GUID + XML_ATTR_OPEN + mGUID.toString() + XML_ATTR_CLOSE); // _GUID="GUID"
		if (mRandomNumber != Double.MIN_VALUE)
			strBuff.append(XML_SPACE + XML_EXCHANGE_RANDOM_NUMBER + XML_ATTR_OPEN + String.valueOf(mRandomNumber) + XML_ATTR_CLOSE); // _RandomNumber="RANDOM NUMBER"
		strBuff.append(XML_SPACE + XML_EXCHANGE_RECURSION + XML_ATTR_OPEN + mRecursion + XML_ATTR_CLOSE); // _Recursion="RECURSION"
		strBuff.append(XML_SPACE + XML_EXCHANGE_LEN_CURRENT + XML_ATTR_OPEN + mLenCurrent + XML_ATTR_CLOSE); // _CurrentLength="LEN_CURRENT"
		strBuff.append(XML_SPACE + XML_EXCHANGE_MINSTORAGE + XML_ATTR_OPEN + mMinStorage + XML_ATTR_CLOSE); // _MinStorage="MinStorage"
		strBuff.append(XML_SPACE + XML_EXCHANGE_REPLICA_ESTIMATE + XML_ATTR_OPEN + mReplicateEstimate + XML_ATTR_CLOSE); // _ReplicaEstimate="REPLICA_ESTIMATE"
		...
		strBuff.append(prefix + XML_ELEMENT_OPEN_END + XML_EXCHANGE + XML_ELEMENT_CLOSE + newLine); // {prefix}</Exchange>{newLine}
